...
export const ReservationTimer: React.FC<ReservationTimerProps> = ({
  ...
  const { minutes, seconds, isExpired, formattedTime, isWarning, isDanger } = 
    useReservationTimer(expiresAt, onExpired)
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <p className="font-medium">
              {isDanger ? '¡Tiempo por agotarse!' : 
               isWarning ? 'Tiempo limitado' : 
               'Tiempo de reserva'}
            </p>
            <p className="text-sm opacity-90">
              Tienes {formattedTime} para completar tu compra
            </p>
          </div>
        </div>
        
        <div className="flex items-center space-x-2">
          <span className={`text-2xl font-bold ${getClockColor()}`}>
            {formattedTime}
          </span>
          {onExtendReservation && (
            <button

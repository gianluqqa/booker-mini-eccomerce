...
describe("Checkout Module - Stock Reservation", () => {
  let testUser: any;
  ...
  describe("POST /checkout/reserve", () => {
    it("should create stock reservation successfully", async () => {
      ...
      expect(reserveStockResponse.body.message).toBe(
    ...
    it("should fail if pending order already exists", async () => {
      ...
      expect(reserveStockResponse.body.data).toHaveProperty(
        "id",
        pendingOrder.id,
      );
    });

...
describe("Cart Module - Shopping Cart", () => {
  let testUser: any;
  ...
  describe("UNKNOWN_ENDPOINT", () => {
    it("should keep cart separate between different users (Isolation)", async () => {
    ...
    it("should validate stock dynamically if book stock changes externally", async () => {
    ...
    it("should handle price changes in catalog correctly", async () => {
      await addToCart(app, authToken, { bookId: testBook.id, quantity: 1 });

      // Cambiamos el precio en el catálogo
      await AppDataSource.getRepository(Book).update(testBook.id, {
        price: 99.99,
      });
    ...
    it("should persist cart after logout and new login", async () => {
    ...
    it("should reject any modification (ADD, PUT, DELETE) if there is pending order", async () => {
      ...
      const resPost = await addToCart(app, authToken, {
        bookId: testBook.id,
        quantity: 1,
      });
    ...
    it("49. debe permitir re-agregar un libro después de haber vaciado el carrito", async () => {
      ...
      const addAgain = await addToCart(app, authToken, {
        bookId: testBook.id,
        quantity: 1,
      });

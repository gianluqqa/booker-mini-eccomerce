...
describe("Reviews Module - Administration", () => {
  let testUser: User;
  ...
  beforeEach(async () => {
    ...
    const adminUser = await createTestUser({
      email: `admin_reviews_${Date.now()}@test.com`,
      role: "admin",
    });
  ...
  describe("GET /reviews/admin/all", () => {
    ...
    it("should return review list with metadata for an administrator", async () => {
    ...
    it("should allow filtering reviews by book title", async () => {
      const res = await getAllReviewsAdmin(app, adminToken, {
        book: "Admin Review",
      });
      expect(res.status).toBe(200);
    ...
    it("should delete any review and return 200", async () => {
